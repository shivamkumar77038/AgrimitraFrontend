...
export default function Login() {
  ...
  const handleLogin = async (data) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/retail/login`, data);
    ...
    <>
     <div className="bg-green-400 flex justify-center items-center w-full h-screen">
        <div className="w-11/12 sm:w-1/2 h-auto bg-green-200 text-green-900 shadow-green-900 shadow-md rounded-2xl p-6">
          <h2 className="text-2xl font-bold text-center mb-6">Retailer Login!</h2>
          <form onSubmit={handleSubmit(handleLogin)} className="space-y-6">
            ...
            <div className="text-center">
              ...
              <Link className="text-green-900 text-xl hover:text-red-700 hover:underline" to="/Retailsignup">
                Sign Up
              </Link>
            </div>
          </form>
